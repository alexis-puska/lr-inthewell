...
Player::Player(int x, int y, int type, unsigned short *in_keystate) : Position(x, y), Drawable(), HitBox()
{
    this->type = type;
...
void Player::doSomething(SDL_Surface *dest, bool *platformGrid)
{
    ...
    else
    {
         ...
         ****************************************/
        if (((hitboxPoint[2] && hitboxPoint[3]) || (hitboxPoint[3] && hitboxPoint[4])) && (y % 20 <= 5 || y % 20 == 0) && (playerFalling || state == playerStartFall))
        {
            if (!hitboxPoint[7])
            {
                //std::cout<<"j'atteris\n";
