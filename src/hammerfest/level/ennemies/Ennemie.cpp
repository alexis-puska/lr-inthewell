...
Ennemie::Ennemie(int id, int x, int y, int type, Level *level) : Position((x * 20) + 10, (y * 20) + 20), Drawable(), HitBox(), IdElement(id)
{
...
int Ennemie::whatITouch()
{
	...
	if (direction == left)
	{
		...
		else if ((getGridValue(getGridPositionX(-5)) && getGridValue(getGridPositionX(-10) - 20) && !getGridValue(getGridPositionX(-10) - 40) && !getGridValue(getGridPositionX(-10) - 60)) || (getGridValue(getGridPositionX(-5)) && getGridValue(getGridPositionX(-10) - 20) && getGridValue(getGridPositionX(-10) - 40) && !getGridValue(getGridPositionX(-10) - 60)))
		{
...
void Ennemie::ennemieJump()
{
	...
	{
		//saute sur une platforme � gauche
		setX(getX() - 5);
		if (jumpCycle >= 8)
		{
			changeState(isAngry ? angry : walk);
	...
	{
		//saute sur une plateforme � droite
		setX(getX() + 5);
		if (jumpCycle >= 8)
		{
			changeState(isAngry ? angry : walk);
