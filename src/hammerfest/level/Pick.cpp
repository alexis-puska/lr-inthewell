	...
	if (visible) {
		switch (direction) {
			...
			case pickToLeft:
				...
				copySurfaceToBackRenderer(rotozoomSurface(Sprite::Instance().getAnimation("pick", 1), 90, 1, 0), dest, (x * gridSize)+leftPadding, (y * gridSize));
				break;
			case pickToBottom:
				//fprintf(stderr,"draw pick tob\n");
				copySurfaceToBackRenderer(rotozoomSurface(Sprite::Instance().getAnimation("pick", 1), 180, 1, 0), dest, (x * gridSize)+leftPadding, (y * gridSize));
				break;
			case pickToRight:
				//fprintf(stderr,"draw pick tor\n");
				copySurfaceToBackRenderer(rotozoomSurface(Sprite::Instance().getAnimation("pick", 1), 270, 1, 0), dest, (x * gridSize)+leftPadding, (y * gridSize));
				break;
		}
