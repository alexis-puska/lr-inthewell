...
void Lock::drawHimself(SDL_Surface * dest) {
	if (locked) {
		copySurfaceToBackRenderer(Sprite::Instance().getAnimation("serrure", 1), dest, x - floor(width / 2), y - height);
	} else {
		copySurfaceToBackRenderer(Sprite::Instance().getAnimation("serrure", 0), dest, x - floor(width / 2), y - height);
	}
}
